...
cd fpl-frontend && pnpm dev                   # http://localhost:5000
```

**Port 5000 is contested on macOS.** AirPlay Receiver (`ControlCenter`) binds it by default and the
Next.js dev server will fail to start. Turn it off: **System Settings → General → AirDrop & Handoff →
AirPlay Receiver → Off**. Confirm with `lsof -nP -i :5000`. `doctor.sh` flags this specifically
because the error message Next prints does not.

## Environment

| Repo | File | Keys |
|---|---|---|
| `fpl-backend` | `.env` | `PORT=5001`, `DATABASE_URL`, `FPL_API_BASE`, `FPL_USER_AGENT`, `FPL_MANAGER_ID`, `SYNC_ENABLED`, `CORS_ORIGIN=http://localhost:5000` |
...
|---|---|---|
| Frontend will not start, `:5000` in use | AirPlay Receiver | `lsof -nP -i :5000` → `ControlCe` |
| Frontend loads, every request fails | backend down, or `NEXT_PUBLIC_API_URL` wrong | `curl -s localhost:5001/health` |
| CORS error in the browser console | `CORS_ORIGIN` not `http://localhost:5000` | backend `.env` |
